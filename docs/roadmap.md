...
| [Phase 5](#phase-5課表) | ③ 課表（含台大課程目錄） | ✅ 程式碼完成／⏳ SQL 待執行、目錄待上傳、手動驗證待跑 |
| [Phase 5B](#phase-5b連結-ntu-cool) | 連結 NTU COOL，作業自動變任務 | ⏳ 等你完成 5B.0 驗證（貼一段 .ics） |
| [Phase 6](#phase-6學分與-gpa) | ④ 學分與 GPA 追蹤 | ✅ 程式碼完成／⏳ 手動驗證待跑（與校務系統比對） |
...
### 產出與和計劃不同之處（2026-09-26）

- **課程目錄的來源改成 NOL**（`nol.ntu.edu.tw`，舊版公開課程查詢）：`course.ntu.edu.tw` 對程式化請求直接回「永久封鎖」。
  腳本 `scripts/catalog/fetch_catalog.py`（Python，依你提供的 Node 版改寫）；115-1 實測解析 16,032／16,032 列、合併成 12,044 門。
- **時段放在課程那一列的 jsonb**，不另開 `course_sessions`：背景寫入不保證順序，兩張表會讓時段先到而被外鍵拒絕。
- **不做 `catalog_terms`**：台大行事曆無法可靠自動取得；改成每學期在課表頁問一次開學日（D30，同步到 `user_settings.term_starts`），所有學校通用。
- ~~目錄只存 NOL 原文時間字串，App 解析~~ → 見下方「多校目錄」：改成腳本讀好時間、存成 `sessions`。
- 資料代號改為 D27 `courses`、D29 `course_catalog`、D30 `term_starts`、D31 畢業學分與身分（D24–D26 已被快取、觸覺回饋、風格使用；D28 保留給 5B）。
- 需要你：執行 `supabase/courses.sql`、`supabase/course_catalog.sql`，設定 `scripts/catalog/.env` 後跑一次腳本。

### 多校目錄（2026-09-26 追加）

**你的要求**：未來要加入各大學的課表，保留擴充性；一支腳本就能控制要哪個學校、哪個學期。

- **一支腳本**：`scripts/catalog/fetch_catalog.py --school=ntu,nthu --semesters=115-1`（`--list` 看有哪些學校；`--school` 必填）。
- **每校一個模組**（`schools/<校>.py`）只負責抓與讀；共同程式負責 id、合併、截斷、完整性、寫入、刪掉已停開的課。
- **時間在腳本讀好**，目錄多一欄 `sessions`；App 刪掉 `parseNtuTime`，不再有任何學校的解析規則。
- **App 從 `catalog_schools`（D32）得知有哪些學校**：新增學校只要跑腳本，不必發新版。
- **節次表**同時在 `scripts/catalog/schools.json` 與 `period_tables.dart`，測試確保一致。
- **接上清大**：官方開放資料（一個 JSON 檔、只有當學期），115-1 共 3,043 門；清大學生手動新增也能選節次。
- 移植時順手修正：台大的「四10」（只有第 10 節）原本會被讀成第 1 節。

### 5.1 使用流程（UC18）

1. **任務頁「今天的課」**：當天有課時，完成度卡上方出現一列時間軸 chip——「10:20 微積分・新數102」；
   **正在上的課加粗、下一堂標「下一堂」**，已經上完的變淡。沒課的日子不顯示這列（不佔空間）。
2. **完整課表**：~~目標頁頁首新增 📅~~ → 2026-09-26 改為任務頁的「課表」卡（手機在完成度卡旁，桌面在其下方）＋側邊欄分隔線下的「課表」→ `TimetableScreen`：
...
**實際（2026-09-26）**：入口搬到任務頁後，改成任務章在完成度卡之後、檢視切換之前加一站 `info`，指向課表卡；學期章不再有課表站。
...
- 百分制換算以官方〈GPA 單向轉換為百分制成績對照表〉為準：在 4.30／4.00／3.70／3.30／3.00／2.70／2.30／2.00／1.70 之間直線內插、四捨五入兩位；
  測試對照印出的表抽 22 點全數吻合。1.70 以下官方表沒有，不顯示。
- 成績頁與課表同一頁（上方切換），GPA 趨勢線、目標試算、畢業學分進度都已完成；學期回顧快照多存 `stats.gpa`。
- 畢業學分與身分存在本機（D31）並單獨同步到 `user_settings`（欄位在 `courses.sql`）。
