...
- **側邊欄**（手機的三條線抽屜、桌面的 `NavigationRail`）在四個分頁下方有一條分隔線，
  線下是以「開新頁」方式進入的次要功能，目前是「課表」。展開的 rail 顯示圖示＋文字，
  收合的 rail 只有圖示（滑過顯示「課表」）。
...
| 輸入 | 元件 | 格式／規則 |
|---|---|---|
| 搜尋{學校}課程 | 「新增課程」裡每個這學期有目錄的學校一列（D32，使用者自己的學校排第一，讀不到就只剩手動）→ sheet，輸入 300ms 後查詢 | 只查那所學校；課名、老師、課號或流水號，至少 2 字；結果顯示時間教室、學分、老師、流水號，與已有的課重疊時紅字「與 X 衝堂」（仍可加）；已加入的標「已加入」 |
| 手動新增／編輯課程 | sheet（狀態在 `_CourseForm` 的 State，不在 builder） | 課名（必填，≤100）、老師（≤50）、學分（0–99.9）、顏色（16 色）、時段清單（星期＋開始／結束＋教室）。有節次表的學校（台大、清大，`kSchoolPeriods`）用節次下拉（「3｜10:20」），其他學校用時間選擇器；結束必須晚於開始 |
...
### 3-S 課表（`core/timetable.dart`、`core/period_tables.dart`）

**節次表**（`kSchoolPeriods`，依使用者的學校名稱查；沒有表的學校用時鐘時間）：
- 台大（教務處官方表，`kNtuPeriods`）：0 07:10、1 08:10、2 09:10、3 10:20、4 11:20、5 12:20、6 13:20、7 14:20、
  8 15:30、9 16:30、10 17:30、A 18:25、B 19:20、C 20:15、D 21:10，每節 50 分鐘。
- 清大（課務組〈星期代碼、各節上課時間〉，`kNthuPeriods`）：1 08:00、2 09:00、3 10:10、4 11:10、n 12:10、5 13:20、6 14:20、
  7 15:30、8 16:30、9 17:30、a 18:30、b 19:30、c 20:30、d 21:30，每節 50 分鐘。
- 同一份表也在 `scripts/catalog/schools.json`（腳本讀時間用），`test/period_tables_test.dart` 確保兩邊一致。

**讀各校的時間字串**：只在腳本裡做（`scripts/catalog/schools/<校>.py`），App 從目錄拿到的就是時段（D29 `sessions`）。
共同規則 `periods_to_sessions`：**依節次表的順序，連續的節次合成一段**，不連續就拆開；表裡沒有的節次丟掉、不猜。
- 台大 NOL：`三6,7 (基醫508)`、`一3,4(新102)四8(普101)`、`五A,B`、`四10`——每段是「星期字＋節次（逗號分隔）＋可有可無的（教室）」；`一2,3,5` → 2–3 與 5；讀不懂的（「請洽系所辦」）不產生時段。
- 清大開放資料：每行「教室\t代碼」，代碼是星期字母（M T W R F S U）＋節次，例如 `T7T8T9Ta` → 週二 15:30–19:20（9 之後接 a，仍是一段）；多行就是多間教室，各自帶教室名稱。
...
**格線**（`gridBounds`）：時間軸至少 08–18，有更早或更晚的課就延伸到整點；週一到週五，有週六或週日的課才多出那一欄。
每個時段以 `Positioned` 依分鐘放置（不用 `IntrinsicHeight`）；點空白格以「那一天、那個整點」開新增課程。

**上課提醒**（`_classReminders`，§3-K 的第五種）：未來 7 天、每天先看那天屬於哪個學期（`termAt`），**那個學期設了開學日而且在上課週內**才排；
提前 5／10／15／30 分鐘；最多 20 則，避免擠掉其他提醒（總上限仍是 48）。

**小工具「課表」分頁**（`_classRows`）：今天還沒上完的課；都上完了就是明天的課（前面一列標頭「明天」）；不在上課週內就沒有。

**課程目錄**（`scripts/catalog/`，見 data_dictionary.md D29、D32 與該資料夾 README）：一支腳本，`--school` 選學校、`--semesters` 選學期，
每校一個模組只負責「從自己的來源抓回一學期、讀成共同格式」，其餘（id、合併授課對象、截斷、完整性、寫入）是共同程式。

```mermaid
flowchart TD
    Start(["fetch_catalog.py --school=ntu,nthu [--semesters=115-1]"]) --> Known{"學校都在 REGISTRY？"}
    Known -->|否| Stop(["列出已知學校，結束"])
    Known -->|是| Sem["每校：指定的學期，或該校預設
（台大：本學期＋上學期；清大：開放資料裡的學期）"]
    Sem --> Fetch["schools/&lt;校&gt;.fetch(學期)
抓來源 → 共同格式，時間經 periods_to_sessions 變成 sessions"]
    Fetch --> Finish["common.finish：id = 校_學期_key、同 key 合併授課對象、截斷到上限"]
    Finish --> Complete{"完整？
台大：解析數 = NOL 總數且沒有失敗頁
清大：該學期有資料"}
    Complete -->|否，且沒有 --allow-partial| Skip["不寫，保留上次的資料"]
    Complete -->|是| Dry{"--dry-run？"}
    Dry -->|是| Report
    Dry -->|否| Write["upsert course_catalog
再刪掉同校同學期、這次沒看到的（已停開）"]
    Write --> School["更新 catalog_schools：把學期加進該校"]
    School --> Report["報表：來源數、解析數、課程數、讀不懂的時間"]
    Skip --> Report
```

新增一所學校：`schools.json` 一筆（名稱、簡稱、節次）＋一個模組＋fixture 測試；跑完腳本，App 的「新增課程」就從 D32 多出那所學校，不用發新版。

### 3-T 成績與 GPA（`core/grade_scale.dart`、`core/gpa_stats.dart`）

依台大教務處〈等第制成績定義與等第績分表〉與〈等第績分平均(GPA)單向轉換為百分制成績對照表〉：

| 等第 | A+ | A | A- | B+ | B | B- | C+ | C | C- | F | X |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 績分 | 4.3 | 4.0 | 3.7 | 3.3 | 3.0 | 2.7 | 2.3 | 2.0 | 1.7 | 0 | 0 |

- **GPA**：學分加權平均，四捨五入到小數兩位；F 與 X 以 0 分計入分母；通過／不通過／停修、還沒給分、關掉「計入 GPA」的都不算。沒有任何成績時是「未知」（`null`），不是 0。
- **及格**（計入已修學分）：學士班 C- 以上、研究所 B- 以上；「通過」算，不通過與停修不算。
- **百分制換算**：官方表是這些點之間的直線：4.30→100、4.00→89、3.70→84、3.30→79、3.00→76、2.70→72、2.30→69、2.00→66、1.70→60，
  每個值四捨五入到小數兩位；1.70 以下官方表沒有，App 也不顯示。為了讓 3.69 → 83.88 這類值與印出的表完全一致，以整數（百分之一）運算。
- **目標試算**（`requiredAverage`）：剩下（還沒給分、計入 GPA、有學分）的課平均需要多少：
  `需要 = (目標 × 總學分 − 已得績分) ÷ 剩下學分`；≤ 0 是「已經達成」；> 4.3 是「做不到」並顯示全拿 A+ 的最高值；沒有剩下的課另有說明。需要值無條件進位到小數兩位，保證夠。
- 學期回顧（§3-P）的數字多一行該學期 GPA，存進快照。
...
flowchart TD
    ...
    B -->|否| C["不算進 GPA"]
    B -->|是| D["績分 × 學分 加進分子\n學分加進分母"]
    D --> E["GPA = 分子 ÷ 分母，四捨五入兩位"]
    A --> F{"及格？\n學士 ≥ C-／研究所 ≥ B-\n或「通過」"}
    F -->|是| G["學分算進已修學分"]
    F -->|否| H["不算"]
```

其他學校可能用不同的等第表；成績頁在非台大時會註明。
...
3. 加課：「新增課程」→
   - **搜尋{學校}課程**（每個這學期有目錄的學校一列，自己的學校排第一）：輸入課名／老師／課號／流水號 → 點結果 → 課與所有時段一次建立；衝堂會先標示；
   - **手動新增**：填課名、時段（有節次表的學校選節次）→ 新增。點格線空白處也能新增，星期與時間已帶入。
4. 點格線上的課 → 編輯（含成績）或刪除（進回收桶，還原時時段一起回來）。
5. 任務頁在當天有課時多一列「今天的課」；通知設定可開關上課前提醒；小工具多一個「課表」分頁。

### UC20　成績與 GPA（Phase 6）

1. 期末在課表點每一門課，選等第（或通過／不通過／停修）。
2. 課表頁切到「成績」：第一次先確認畢業學分與身分。
3. 看本學期 GPA、累積 GPA 與百分制、已修學分進度；有兩個學期以上的成績時出現趨勢線。
4. 輸入目標累積 GPA，看剩下的課平均要多少。
5. 學期回顧會多一行該學期 GPA。
